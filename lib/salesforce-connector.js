...
var Salesforce = function(config) {
    ...
    return {
        ...
        getPatronIdByName: function(patron_name, cb) {
            conn.query("SELECT Id FROM patron__c WHERE Name = '"+patron_name+"'", function(err, result) {
                if (err) {
                    console.log(err);
        ...
        /**
         * Retrieves the high scores globally, decreasing order, top 10 records
         */
        ...
        /**
         * Retrieves the high scores for a given location, decreasing order, top 10 records
         */
        getHighScoresByLocation: function(account_id, cb) {
            conn.query("SELECT points__c, patron__r.Name FROM score__c WHERE account__c = '"+account_id+"' ORDER BY points__c DESC LIMIT 5", function(err, result) {
                if (err) {
                    console.log(err);
                    throw "Cannot retrieve Scores";
                }
